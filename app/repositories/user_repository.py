...
class UserRepository(Repository):
    ...
    def get_all(self, db_cursor:Cursor) -> list:
        app_logger.info("listing all users")
        list_users_query = """SELECT * FROM users;"""
        user_list = db_cursor.execute(list_users_query).fetchall()
        return user_list

    def get_one(self, db_cursor:Cursor, user_id:UUID) -> dict:
        app_logger.info(f"get user details for user_id: {user_id}")
        get_user_query = f""" SELECT * FROM users WHERE user_id = %(user_id)s;"""
        
        user_data = db_cursor.execute(get_user_query, {'user_id':user_id.hex}).fetchone()
        return user_data

    def create(self, db_cursor:Cursor, user: User):
        ...
        db_cursor.execute(create_user_query, user_model_dump)
        return 
    
    def update(self, db_cursor:Cursor, user:User):
...
"""
        ...
        db_cursor.execute(update_user_query, user_model_dump)
        return
    
    def update_part(self, db_cursor:Cursor, user_id:UUID, update_dict:dict):
...
""".format(", ".join([f"{key} = {value}" for key,value in update_dict.items()]))
        ...
        db_cursor.execute(update_partial_user_query, update_dict)
        return

    def delete(self, db_cursor:Cursor, user_id: UUID):
        app_logger.info(f"Deleting user with user_id: {user_id}")
        delete_user_query = """DELETE FROM users WHERE user_id = %(user_id)s"""
        
        db_cursor.execute(delete_user_query, {"user_id":user_id.hex})
        return
    
    def append_to_wallets(self, db_cursor:Cursor, user_id:UUID, wallet_id: UUID):
...
"""
        ...
        db_cursor.execute(append_to_wallet_query, {"user_id":user_id, "wallet_id":wallet_id})
        return
    
    def append_to_family_members(self, db_cursor:Cursor, user_id:UUID, family_member_id:UUID):
...
"""
        ...
        db_cursor.execute(append_to_family_members_query, {"user_id":user_id, "family_member_id":family_member_id})
        return
    
    def delete_wallet(self, db_cursor:Cursor, user_id:UUID, wallet_id:UUID):
...
"""
        try:
            db_cursor.execute(delete_from_wallet_list_query, {"user_id":user_id, "wallet_id":wallet_id})
