...
class WalletRepository(Repository):
    ...
    def get_all(self, db_cursor:Cursor) -> list:
        ...
        user_list = db_cursor.execute(list_wallets_query).fetchall()
        return user_list
    
    def get_one(self, db_cursor:Cursor, wallet_id: UUID) -> dict:
        ...
        wallet_data = db_cursor.execute(get_wallet_query, {"wallet_id":wallet_id}).fetchone()
        return wallet_data
    
    def create(self, db_cursor:Cursor, wallet:Wallet):
        ...
        db_cursor.execute(create_wallet_query, wallet_model_dump)
        return
        
    
    def update(self, db_cursor:Cursor, wallet:Wallet):
...
"""
        db_cursor.execute(update_wallet_query, wallet_model_dump)
        return

    def update_part(self):
        pass

    def delete(self, db_cursor:Cursor, wallet_id:UUID):
        ...
        db_cursor.execute(delete_wallet_query, {"wallet_id":wallet_id})
        return
